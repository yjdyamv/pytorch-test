...
def main(args):
    ...
    if batch_size is None:
        if device.type == "cuda":
            ...
            if total_mem_gb > 7:
                ...
                batch_size = 8
            print(
                f"Auto-detected batch_size: {batch_size} (GPU mem: {total_mem_gb:.1f}GB)"
            )
        else:
            batch_size = 8
            print(f"Default batch_size: {batch_size} (CPU mode)")
    nw = min(
        [os.cpu_count() or 1, batch_size if batch_size > 1 else 0, 8]
    )  # number of workers
    ...
    checkpoint = None
    if args.resume:
        assert os.path.exists(args.resume), "resume file: '{}' not exist.".format(
            args.resume
        )
        ...
        print(
            "Resumed from checkpoint: '{}' (epoch {})".format(
...
if __name__ == "__main__":
    ...
    # 断点续训权重路径
    parser.add_argument(
        "--resume", type=str, default="", help="resume from checkpoint path"
    )
    parser.add_argument(
